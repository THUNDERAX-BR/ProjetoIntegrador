...
public class CadastroAutor extends javax.swing.JPanel {
     ...
     */
    public CadastroAutor(GlobalInterface origem, int id) {
        initComponents();
        this.id = id;
    ...
    private void BtSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtSalvarActionPerformed
        if(!TxNome.getText().isEmpty()&&CbMovimento.getSelectedIndex()!=-1&&verificarData()&&!TxBiografia.getText().isEmpty()){
            ...
            String falecimento = null;
            if(CbVivo.getSelectedIndex()==0){
                falecimento = TxFalecimento.getText();
            }
            String[] itemdividido = item.split("/");
            int idmov = Integer.parseInt(itemdividido[0]);
            AutoresDAO dao = new AutoresDAO(Conector.conectar());
            if(id==-1){
                dao.cadastrarAutor(TxNome.getText(), idmov, TxNascimento.getText(), falecimento, TxBiografia.getText(), imagem);
            }
            else{
                dao.alterarAutor(id, TxNome.getText(), idmov, TxNascimento.getText(), falecimento, TxBiografia.getText(), imagem);
            }
    ...
    private void atualizar(){
        ...
        MovimentosDAO mdao = new MovimentosDAO(Conector.conectar());
        ArrayList<String> lista = mdao.listarCadastroAutor(id);
        for(String s : lista){
        ...
        if(id!=-1){
            ...
            AutoresDAO dao = new AutoresDAO(Conector.conectar());
            Autores a = dao.exibirAutor(id);
            TxNome.setText(a.getNome());
            TxNascimento.setText(a.getDataNascimento());
            if(a.getDataFalecimento()!=null){
                TxFalecimento.setText(a.getDataFalecimento());
            }
            else{
                CbVivo.setSelectedIndex(1);
            }
            TxBiografia.setText(a.getBiografia());
            if(a.getFoto()!=null){
    ...
    private boolean verificarData(){
        ...
        if(TxNascimento.isEditValid()&&(TxFalecimento.isEditValid()||CbVivo.getSelectedIndex()==1)){
            try {
                sdf.parse(TxNascimento.getText());
                if(CbVivo.getSelectedIndex()==0){
                    sdf.parse(TxFalecimento.getText());
                }
                return true;
