...
public class CadastroLivro extends javax.swing.JPanel {
     ...
     */
    public CadastroLivro(GlobalInterface origem, int id) {
        initComponents();
        this.id = id;
    ...
    private void BtCapaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtCapaActionPerformed
        ...
        int resultado = select.showOpenDialog(null);
    ...
    private void BtCategoriasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtCategoriasActionPerformed
            ...
            Categorias c = new Categorias(Integer.parseInt(itemdividido[0]), itemdividido[1]);
    ...
    private void BtSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtSalvarActionPerformed
        if(!TxTitulo.getText().isEmpty()&&!TxAno.getText().isEmpty()&&!TxSinopse.getText().isEmpty()&&CbAutor.getSelectedIndex()!=-1&&CbEditora.getSelectedIndex()!=-1){
            ...
            String[] itemdividido = item.split("/");
            int idaut = Integer.parseInt(itemdividido[0]);
            item = (String) CbEditora.getSelectedItem();
            itemdividido = item.split("/");
            int idedi = Integer.parseInt(itemdividido[0]);
            LivrosDAO dao = new LivrosDAO(Conector.conectar());
            if(id==-1){
                dao.cadastrarLivro(TxTitulo.getText(), Integer.parseInt(TxAno.getText()), idaut, idedi, TxSinopse.getText(), imagem, categorias);
            }
            else{
                dao.alterarLivro(id, TxTitulo.getText(), Integer.parseInt(TxAno.getText()), idaut, idedi, TxSinopse.getText(), imagem, categorias);
            }
    ...
    private void atualizar(){
        ...
        AutoresDAO adao = new AutoresDAO(Conector.conectar());
        ArrayList<String> lista = adao.listarCadastroLivro(id);
        for(String s : lista){
            CbAutor.addItem(s);
        }
        CbEditora.removeAllItems();
        EditorasDAO edao = new EditorasDAO(Conector.conectar());
        ArrayList<String> lista2 = edao.listarCadastroLivro(id);
        for(String s : lista2){
            CbEditora.addItem(s);
        }
        CategoriasDAO cdao = new CategoriasDAO(Conector.conectar());
        ArrayList<String> lista3 = cdao.listarCadastroLivro();
        for(String s : lista3){
            CbCategoria.addItem(s);
        }
        CbCategoria.setSelectedIndex(-1);
        if(id!=-1){
            cdao.reconectar(Conector.conectar());
            categorias = cdao.categoriasLivro(id);
            CbAutor.setSelectedIndex(0);
            CbEditora.setSelectedIndex(0);
            LivrosDAO dao = new LivrosDAO(Conector.conectar());
            Livros l = dao.exibirLivro(id);
            TxTitulo.setText(l.getTitulo());
            TxAno.setText(String.valueOf(l.getAno()));
